"""Predictive-maintenance Phase-6 insight pipeline.
...
log = logging.getLogger("fieldbridge.predictive_maintenance.insights")

MODULE_SLUG = "predictive_maintenance"


def _build_data_context(engine: Engine, tenant_id: str) -> dict[str, Any]:
    """Compact predictive-maintenance context for the LLM.

    Mirrors the JSON keys documented in
    :data:`app.modules.predictive_maintenance.prompts.SYSTEM_PROMPT`.
    """
    ...
    return {
        ...
        "risk_tier_breakdown": insights_json.get("risk_tier_breakdown", {}),
        "status_breakdown": insights_json.get("status_breakdown", {}),
        "source_breakdown": insights_json.get("source_breakdown", {}),
        "failure_mode_breakdown": insights_json.get(
            "failure_mode_breakdown", {},
        ),
        "aging_breakdown": insights_json.get("aging_breakdown", {}),
        "top_equipment_exposure": insights_json.get(
            "top_equipment_exposure", [],
        ),
        "failure_mode_impact": insights_json.get("failure_mode_impact", []),
        "top_by_exposure": insights_json.get("top_by_exposure", []),
        "recent_completions": insights_json.get("recent_completions", []),
    }
...
def _load_cached(
    ...
    try:
        ...
        log.warning(
            "dropping unparseable cache row for predictive_maintenance: %s",
            exc,
        )
        return None
...
def _upsert_cache(
    ...
    try:
        ...
        log.warning(
            "predictive_maintenance insight cache write failed: %s", exc,
        )
