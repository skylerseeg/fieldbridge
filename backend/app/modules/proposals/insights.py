"""Proposals Phase-6 insight pipeline.
...
log = logging.getLogger("fieldbridge.proposals.insights")

MODULE_SLUG = "proposals"


def _build_data_context(engine: Engine, tenant_id: str) -> dict[str, Any]:
    """Compact proposal-strategy context for the LLM.

    Mirrors the JSON keys documented in
    :data:`app.modules.proposals.prompts.SYSTEM_PROMPT`.
    """
    ...
    return {
        ...
        "primary_state": service.DEFAULT_PRIMARY_STATE,
        "bid_type_category_breakdown": insights_json.get(
            "bid_type_category_breakdown", {},
        ),
        "geography_tier_breakdown": insights_json.get(
            "geography_tier_breakdown", {},
        ),
        "top_owners": insights_json.get("top_owners", []),
        "top_bid_types": insights_json.get("top_bid_types", []),
        "top_counties": insights_json.get("top_counties", []),
        "top_states": insights_json.get("top_states", []),
        "competitor_frequency": insights_json.get("competitor_frequency", []),
        "fee_statistics": insights_json.get("fee_statistics", []),
    }
...
def _load_cached(
    ...
    try:
        ...
        log.warning("dropping unparseable cache row for proposals: %s", exc)
        return None
...
def _upsert_cache(
    ...
    try:
        ...
        log.warning("proposals insight cache write failed: %s", exc)
