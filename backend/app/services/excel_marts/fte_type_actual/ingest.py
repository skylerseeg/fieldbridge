"""fte_type_actual IngestJob — Job type FTE actual.xlsx."""
...
from app.services.excel_marts.fte_type_actual.schema import TABLE_NAME
...
def _type_map() -> dict[str, type]:
    t: dict[str, type] = {
        "job_type": str, "code": str, "craft_class": str,
...
job = register_job(
    IngestJob(
        name="fte_type_actual",
        source_file="Job type FTE actual.xlsx",
        target_table=TABLE_NAME,
        sheet_name="FTE Projections",
        column_map=_column_map(),
        type_map=_type_map(),
        dedupe_keys=["tenant_id", "job_type"],
    )
)
